...
const TrendAnalytics = () => {
  ...
  const mockComparisonData = [
    { name: 'Smart Building IoT', score: 4.2, phase: 'Growing', impact: 4.5, relevance: 4.2, urgency: 3.8, feasibility: 3.9, risk: 2.1 },
    { name: 'Virtual Tours', score: 3.8, phase: 'Growing', impact: 4.1, relevance: 3.8, urgency: 3.2, feasibility: 4.3, risk: 2.8 },
    { name: 'Sustainable Dev.', score: 4.6, phase: 'Mainstream', impact: 4.8, relevance: 4.6, urgency: 4.2, feasibility: 4.0, risk: 1.8 },
    { name: 'Blockchain Prop.', score: 3.2, phase: 'Emerging', impact: 3.5, relevance: 3.2, urgency: 2.8, feasibility: 2.5, risk: 3.5 },
    { name: 'Traditional Offices', score: 2.8, phase: 'Declining', impact: 2.1, relevance: 2.8, urgency: 2.0, feasibility: 4.5, risk: 4.2 }
  ]
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
        ...
        {/* Comparison Tab - FIXED WITHOUT RADAR CHART */}
        <TabsContent value="comparison" className="space-y-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Vergleich der Priority Scores aller Trends
                </CardDescription>
              </CardHeader>
              <CardContent>
                <div className="h-80">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={mockComparisonData} layout="horizontal">
                      <CartesianGrid strokeDasharray="3 3" />
                      <XAxis type="number" domain={[0, 5]} />
                      <YAxis dataKey="name" type="category" width={100} />
                      <Tooltip />
                      <Bar dataKey="score" fill="#3B82F6" name="Priority Score" />
                    </BarChart>
                  </ResponsiveContainer>
                </div>
              </CardContent>
            ...
            <Card>
              <CardHeader>
                <CardTitle>Impact vs. Priority Score</CardTitle>
                <CardDescription>
                  Vergleich von Impact und Priority Score
                </CardDescription>
              </CardHeader>
              <CardContent>
                <div className="h-80">
                  <ResponsiveContainer width="100%" height="100%">
                    <ScatterChart data={mockComparisonData}>
                      <CartesianGrid strokeDasharray="3 3" />
                      <XAxis 
                        type="number" 
                        dataKey="score" 
                      ...
                      />
                      <Tooltip 
                        formatter={(value, name) => [value.toFixed(1), name === 'score' ? 'Priority Score' : 'Impact']}
                        labelFormatter={(label, payload) => payload?.[0]?.payload?.name || ''}
                      />
                      <Scatter 
                        dataKey="impact" 
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Multidimensionaler Vergleich aller Trend-Bewertungen
              </CardDescription>
            </CardHeader>
            <CardContent>
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b">
                      ...
                      <th className="text-center py-2">Relevanz</th>
                      <th className="text-center py-2">Dringlichkeit</th>
                      <th className="text-center py-2">Machbarkeit</th>
                      <th className="text-center py-2">Risiko</th>
                      <th className="text-center py-2">Phase</th>
                    </tr>
                  </thead>
                  <tbody>
                    {mockComparisonData.map((trend, index) => (
                      <tr key={index} className="border-b hover:bg-gray-50">
                        <td className="py-3 font-medium">{trend.name}</td>
                        <td className="text-center py-3">
                          <Badge className={getScoreColor(trend.score)}>{trend.score}</Badge>
                        </td>
                        <td className="text-center py-3">
                          <Badge className={getScoreColor(trend.impact)}>{trend.impact}</Badge>
                        </td>
                        <td className="text-center py-3">
                          <Badge className={getScoreColor(trend.relevance)}>{trend.relevance}</Badge>
                        </td>
                        <td className="text-center py-3">
                          <Badge className={getScoreColor(trend.urgency)}>{trend.urgency}</Badge>
                        </td>
                        <td className="text-center py-3">
                          <Badge className={getScoreColor(trend.feasibility)}>{trend.feasibility}</Badge>
                        </td>
                        <td className="text-center py-3">
                          <Badge className={getScoreColor(5 - trend.risk)}>{trend.risk}</Badge>
                        </td>
                        <td className="text-center py-3">
                          <Badge className={
                            trend.phase === 'Mainstream' ? 'bg-green-100 text-green-800' :
                            trend.phase === 'Growing' ? 'bg-blue-100 text-blue-800' :
                            trend.phase === 'Emerging' ? 'bg-yellow-100 text-yellow-800' :
              ...
              <CardContent>
                <div className="space-y-4">
                  <div className="p-4 border-l-4 border-blue-500 bg-blue-50">
                    <h4 className="font-medium text-blue-900 mb-2">Starke Korrelation entdeckt</h4>
                    <p className="text-sm text-blue-800">
                      Smart Building IoT und Energy Efficiency zeigen eine Korrelation von 85%. 
                      Investitionen in IoT-Technologien könnten Energieeffizienz-Projekte verstärken.
                    </p>
                  </div>
                  
                  <div className="p-4 border-l-4 border-green-500 bg-green-50">
                    <h4 className="font-medium text-green-900 mb-2">Wachstumschance</h4>
                    <p className="text-sm text-green-800">
                      Sustainable Development befindet sich im Übergang zur Mainstream-Phase. 
                      Jetzt ist der optimale Zeitpunkt für strategische Investitionen.
                    </p>
                  </div>
                  
                  <div className="p-4 border-l-4 border-orange-500 bg-orange-50">
                    <h4 className="font-medium text-orange-900 mb-2">Risiko-Warnung</h4>
                    <p className="text-sm text-orange-800">
                      Traditional Offices zeigen anhaltenden Rückgang. 
                      Diversifikation in flexible Arbeitsmodelle empfohlen.
                    </p>
                  </div>
                </div>
              </CardContent>
